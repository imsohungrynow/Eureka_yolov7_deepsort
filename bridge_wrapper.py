...
class YOLOv7_DeepSORT:
    ...
    def track_video(self,video:str, output:str, skip_frames:int=0, show_live:bool=False, count_objects:bool=False, verbose:int = 0):
        ...
        if output: # get video ready to save locally if flag is set
            ...
            codec = cv2.VideoWriter_fourcc(*"XVID") # H264 사용
            out = cv2.VideoWriter(output, codec, fps, (width, height))

        frame_num = 0
        save_counter = 0
        save_interval = 1000
        output_folder = "track_result"
        ...
        while True: # while video is running
            ...
            for track in self.tracker.tracks:  # update new findings AKA tracks
                ...
                cv2.rectangle(frame, (int(bbox[0]), int(bbox[1]-30)), (int(bbox[0])+(len(class_name)+len(str(track.track_id)))*15, int(bbox[1])), color, -1)
                cv2.putText(frame, class_name + " : " + str(track.track_id),(int(bbox[0]), int(bbox[1]-11)),0, 0.6, (255,255,255),1, lineType=cv2.LINE_AA)

                CX = (bbox[0] + bbox[2]) // 2
                CY = (bbox[1] + bbox[3]) // 2
                track.centroidarr.append((CX, CY))
                # track.centroidarr = track.centroidarr[-self.max_frame_to_keep : ]


                
                if class_name == "person":
                    data.append([frame_num, track.track_id, CX, CY])
            ...
            if frame_num % save_interval == 0:
                save_counter += 1
                current_output_path = os.path.join(output_folder, f"path_{save_counter}.xlsx")
                rst = pd.DataFrame(data, columns=columns)
                rst.to_excel(current_output_path, index=False)
                data = []
                
            # if frame_num % save_interval == 0:
            #     if frame_num % (2 * save_interval) != skip_interval:
            #         save_counter += 1
            #         current_output_path = os.path.join(output_folder, f'path_{save_counter}.xlsx')
            #         rst = pd.DataFrame(data, columns=columns)
            #         rst.to_excel(current_output_path, index=False)
